...
function AutoSaveIndicator({ accounts, reps }: { accounts: any[]; reps: any[] }) {
  ...
  return (
    <div
      className={`flex items-center justify-center h-7 w-7 rounded-full transition-all duration-500 border ${
        isSaving
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {reminder.reason && (
          <div
            className="text-[10px] text-slate-500 italic truncate mt-0.5"
...
export function AppShell({ children }: { children: React.ReactNode }) {
  const { globalMonths, setGlobalMonths, accounts, reps, fetchData, subscribeRealtime } =
    useStore();
    ...
    const checkReminders = () => {
      ...
      activeReminders.forEach((acc) => {
        ...
        if (rDate <= now) {
          ...
          const shouldNotify =
            !lastTime || now.getTime() - new Date(lastTime).getTime() >= 3 * 60 * 60 * 1000;
  ...
  const completedReminders = useMemo(() => {
    return accounts
      ...
      .sort(
        (a, b) => new Date(b.reminderDate || 0).getTime() - new Date(a.reminderDate || 0).getTime(),
      )
      .slice(0, 10);
  }, [accounts]);
        ...
        <div className="flex h-16 items-center gap-4 px-4 md:px-6">
          <Link to="/" className="flex items-center gap-3">
            <img
              src="/app-logo.png"
              alt="Logo"
              className="h-14 w-auto rounded-xl object-contain bg-slate-50/50 p-0.5 shadow-sm"
            />
            <div className="hidden sm:block">
              <div className="text-lg font-bold tracking-tight text-gradient-brand">
                Storefries Sales
              </div>
          ...
          <div className="ml-auto flex items-center gap-2 md:gap-3">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button
                  ...
                  className={cn(
                    ...
                    overdue.length > 0 &&
                      "animate-pulse text-amber-600 hover:text-amber-700 bg-amber-50",
                  )}
              ...
              <PopoverContent
                className="w-80 p-0 rounded-xl border border-slate-200 shadow-xl bg-white/95 backdrop-blur-sm z-50"
                align="end"
              >
                {/* Header */}
