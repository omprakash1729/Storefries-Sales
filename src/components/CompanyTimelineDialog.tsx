...
export function CompanyTimelineDialog() {
  const { accounts, deleteAccount, updateAccount, activeCompanyTimeline, setActiveCompanyTimeline } = useStore();
  ...
  return (
    <Dialog open={activeCompanyTimeline !== null} onOpenChange={(open) => !open && setActiveCompanyTimeline(null)}>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <DialogDescription className="text-[11px] text-muted-foreground font-medium uppercase tracking-wider flex items-center gap-1.5">
                <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse" /> Account Activity & Timeline
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
          ...
          <div className="grid grid-cols-2 gap-4 bg-slate-50/60 p-4 rounded-xl border border-slate-200/50">
            ...
            <div className="space-y-1">
              ...
              <div className="pt-0.5">
                <StatusBadge status={activeCompany.mostRecent.status} className="scale-105 origin-left" />
              </div>
            </div>
            <div className="space-y-1">
                ...
                <div key={h.id} className="relative group/timeline">
                  ...
                  <div className="bg-white border border-slate-200/70 shadow-[0_2px_8px_rgba(0,0,0,0.02)] rounded-xl p-3.5 group-hover/timeline:border-slate-300 group-hover/timeline:shadow-sm transition-all duration-300 relative">
                    ...
                    {h.reminderType && h.reminderType !== "none" && h.reminderDate && (
                      <div className={`mt-2 flex items-center justify-between text-xs p-2 rounded-lg border ${
                        h.reminderClosed 
                          ? "bg-slate-50 text-slate-400 border-slate-200/60" 
                          : "bg-amber-50/50 text-amber-800 border-amber-200/60"
                      }`}>
                        <div className="flex items-center gap-1.5 font-medium">
                          <Clock className={`h-3.5 w-3.5 ${h.reminderClosed ? "text-slate-400" : "text-amber-500"}`} />
                          <span>
                            {h.reminderType === "reach_again" ? "Reach Again" : "Follow Up"} Reminder:{" "}
                            {format(new Date(h.reminderDate), "MMM dd, yyyy")}
                          </span>
                        </div>
                        <button
