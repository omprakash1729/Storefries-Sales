...
export function computeMetrics(accounts: Account[]): Metrics {
  ...
  m.conversion =
    m.total === 0 ? 0 : Math.round(((m.demo + m.trial) / Math.max(1, m.total)) * 1000) / 10;
  return m;
}
...
export function KpiCard({
  ...
  accent?: string;
  progress?: number;
  theme?: "indigo" | "violet" | "slate" | "blue" | "teal" | "amber" | "rose" | "brand";
  onClick?: () => void;
  active?: boolean;
}) {
  const themes = {
    teal: {
      ...
      bg: "bg-teal-50/20",
      glow: "shadow-[0_8px_30px_rgba(20,184,166,0.03)]",
      text: "text-teal-600",
      bar: "bg-teal-500",
      dot: "bg-teal-500",
    },
    violet: {
      ...
      bg: "bg-violet-50/20",
      glow: "shadow-[0_8px_30px_rgba(139,92,246,0.03)]",
    ...
    indigo: {
      ...
      bg: "bg-indigo-50/20",
      glow: "shadow-[0_8px_30px_rgba(99,102,241,0.03)]",
    ...
    slate: {
      ...
      bg: "bg-slate-50/20",
      glow: "shadow-[0_8px_30px_rgba(148,163,184,0.03)]",
      text: "text-slate-600",
      bar: "bg-slate-400",
      dot: "bg-slate-400",
    },
    blue: {
      ...
      bg: "bg-sky-50/20",
      glow: "shadow-[0_8px_30px_rgba(14,165,233,0.03)]",
      text: "text-sky-600",
      bar: "bg-sky-500",
      dot: "bg-sky-500",
    },
    amber: {
      ...
      bg: "bg-amber-50/20",
      glow: "shadow-[0_8px_30px_rgba(245,158,11,0.03)]",
      text: "text-amber-600",
      bar: "bg-amber-500",
      dot: "bg-amber-500",
    },
    rose: {
      ...
      bg: "bg-rose-50/20",
      glow: "shadow-[0_8px_30px_rgba(244,63,94,0.03)]",
      text: "text-rose-600",
      bar: "bg-rose-500",
      dot: "bg-rose-500",
    },
    brand: {
      ...
      bg: "bg-emerald-50/20",
      glow: "shadow-[0_8px_30px_rgba(16,185,129,0.03)]",
  ...
  return (
    ...
    >
      ...
      <div className={`mt-1.5 text-3xl font-extrabold tracking-tight ${accent ?? t.text}`}>
        {value}
      </div>
...
export function RepAvatar({ name }: { name: string }) {
  ...
  return (
    <div
      className={`flex h-7 w-7 items-center justify-center rounded-full text-[10px] font-bold text-white ${cls}`}
    >
      {repInitials(name)}
    </div>
  );
...
export function RepChip({ name }: { name: string }) {
  ...
  return (
    ...
    >
      <span
        className={`h-1.5 w-1.5 rounded-full ${rep ? REP_COLOR_CLASS[rep.color] : "bg-slate-400"}`}
      />
      {name}
    </span>
  );
