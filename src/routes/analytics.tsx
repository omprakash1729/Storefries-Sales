...
export const Route = createFileRoute("/analytics")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Industry distribution, rep performance, and month comparison.",
      },
    ],
  }),
...
function AnalyticsPage() {
  ...
  const accounts = useMemo(
    () => groupAccountsByCompany(rawAccounts).map((u) => u.mostRecent),
    [rawAccounts],
  );
  const allAccounts = useMemo(
    () => groupAccountsByCompany(rawAllAccounts).map((u) => u.mostRecent),
    [rawAllAccounts],
  );
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Deeper view of pipeline, industry, and rep performance.
        </p>
      </div>
            ...
            <Bar
              data={{
                ...
                datasets: [
                  {
                    label: "Prospect",
                    data: repBreakdown.map((r) => r.prospect),
                    backgroundColor: STATUS_COLORS.prospect,
                  },
                  {
                    label: "Demo",
                    data: repBreakdown.map((r) => r.demo),
                    backgroundColor: STATUS_COLORS.demo,
                  },
                  {
                    label: "Trial",
                    data: repBreakdown.map((r) => r.trial),
                    backgroundColor: STATUS_COLORS.trial,
                  },
                  {
                    label: "Rejected",
                    data: repBreakdown.map((r) => r.rejected),
          ...
          <Bar
            data={{
              ...
              datasets: [
                {
                  label: "Prospect",
                  data: monthCompare.map((m) => m.prospect),
                  backgroundColor: STATUS_COLORS.prospect,
                },
                {
                  label: "Demo",
                  data: monthCompare.map((m) => m.demo),
                  backgroundColor: STATUS_COLORS.demo,
                },
                {
                  label: "Trial",
                  data: monthCompare.map((m) => m.trial),
                  backgroundColor: STATUS_COLORS.trial,
                },
                {
                  label: "Rejected",
                  data: monthCompare.map((m) => m.rejected),
                  backgroundColor: STATUS_COLORS.rejected,
                },
              ],
            }}
