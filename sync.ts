...
async function run() {
  ...
  const { data: existing, error: fetchErr } = await supabase.from("sales_accounts").select("createdAt");
  ...
  const toInsert = SEED_ACCOUNTS
    .filter(a => !!a.createdAt && !existingDates.has(a.createdAt))
    .map(({ id, ...rest }) => ({
       ...rest,
